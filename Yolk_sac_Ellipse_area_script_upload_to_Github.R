#This script uses the morphometric measurment dataset to analyze the mean yolk sac area for sablefish larvae in each treatment on each experimental day.
...
#We used the equation for the area of an ellipse, the best approximation of a yolk sac, to calculate yolk sac area:
#A=π*  l/2*  w/2
...
#load the morphometric dataset
load("morpho_dat.start.rda")


#Multiply 1/2yolk sac length by 1/2yolk sac depth by pi for each larva
morpho_dat.start$Yolk.Sac.Ellipse_Area<-(morpho_dat.start$YSL/2)*(morpho_dat.start$YSD/2) * pi

#Create character values for the temperature treatments for ease of labelling
morpho_dat.start$Temp<-ifelse(morpho_dat.start$Temp == 6, "6°C","9°C")

#Create a dataframe of the mean daily yolk sac area for sablefish in each treatment
morpho_dat.start_YS_Ellipse_mean<-as.data.frame(morpho_dat.start%>%
                                          group_by(exp_day, Fed, Temp, Rep)%>%
                                          dplyr::summarize(n = n(),
                                                           mean_Ellipse_Area = mean(Yolk.Sac.Ellipse_Area, na.rm =T),
                                                           SE_Ellipse_Area = (2*sd(Yolk.Sac.Ellipse_Area, na.rm =T)/sqrt(n))))
...
# ggsave("Yolk_sac_ellipse_area_plot.png", Yolk_sac_ellipse_area_plot,
#        height = 8,
#        width = 8,
#        dpi = 300,
