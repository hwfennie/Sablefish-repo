...
#This script uses a variety of models and model selection to understand the importance of temperature and feeding treatments on the daily length trajectories of larval sablefish.
#Weight was measured in groups of fish and they average weight of fish in that group was assigned to each individual fish in that group. Mean daily weight for a given group was the average of all fish measured (after being assigned mean group weight) in that treatment.
...
#Plot
GAM.check.weight_gam_global_smooth_plus_group_level_shared_smooth_plus_smooth_interaction_plus_random_effect.gam.check_plot<-ggplot(filter(GAM.check.weight_gam_global_smooth_plus_group_level_shared_smooth_plus_smooth_interaction_plus_random_effect, !(.smooth=="s(Rep)")), aes(x = exp_day, y = .estimate, fill = Fed))+
